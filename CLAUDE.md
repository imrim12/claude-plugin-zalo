...
| `core/daemon-ensure.ts` | Heartbeat-based liveness → spawn the detached daemon on demand (`spawnDaemon`, `windowsHide`, stdio→`daemon.log`). The ONLY daemon-start path — there is no Scheduled Task. `ZALO_NO_DAEMON_SPAWN=1` disables spawn (tests). |

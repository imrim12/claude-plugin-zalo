...
// session dies, its responses are never claimed and expire via dbPrune — fail-closed
// (objection A10): a reply never falls through to another session.
import { z } from 'zod'
import { randomBytes } from 'crypto'
import { mcp } from '../core/mcp.ts'
import { permRequestCreate, outboundCreate, permResponseList } from '../core/db/index.ts'

const myRequests = new Set<string>()
let sessionId = ''

export function permissionRelay(sid: string): void {
  sessionId = sid
  mcp.setNotificationHandler(
    ...
    async ({ params }) => {
      ...
      permRequestCreate(request_id, sessionId)
      myRequests.add(request_id)
      // The daemon owns the Zalo connection and fans this out to every allowlisted DM.
      outboundCreate({ kind: 'permission_dm', idem_key: randomBytes(8).toString('hex'), text })
    },
  )
}

export function permissionPoll(): void {
  setInterval(() => {
    if (myRequests.size === 0) return
    for (const { request_id, behavior } of permResponseList([...myRequests])) {

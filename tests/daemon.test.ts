...
function q(script: string): string {
  const p = Bun.spawnSync(["bun", "-e", `import('./src/core/db/index.ts').then(async m=>{m.db();${script}})`], {
    cwd: PROJECT_ROOT,
...
beforeAll(async () => {
  ...
  await until(() => q("console.log(m.metaGet('heartbeat')?'1':'0')") === "1");
}, 20_000);
...
describe("daemon integration (ZALO_FAKE)", () => {
  test("a synthetic inbound DM becomes a should_reply messages row", async () => {
    ...
    const ok = await until(() => q("const r=m.messageGet('dm1');console.log(r&&r.should_reply===1?'1':'0')") === "1");
    expect(ok).toBe(true);
  }, 15_000);
  ...
  test("an outbound reply is drained to sent and watermark-processes the chat", async () => {
    ...
    const rowId = q("const r=m.messageGet('dm1');console.log(r?r.id:'')");
    expect(rowId).not.toBe("");
    q(`m.outboundCreate({kind:'reply',idem_key:'itest',chat_id:'u123',thread_type:'user',watermark_id:${rowId},payload:JSON.stringify({chunks:['ok']})});console.log('enq')`);
    ...
    const processed = q("const r=m.messageGet('dm1');console.log(r&&r.processed===1?'1':'0')");

...
import {
  db, messageCreate, messageClaim, messageList, outboundCreate,
  outboundUpdate, outboundGet, messageUpdate, messageGet,
} from "../src/core/db/index.ts";
...
function ins(chat: string, opts: Partial<Parameters<typeof messageCreate>[0]> = {}) {
...
describe("db", () => {
  ...
  test("messageClaim returns disjoint sets for two sessions", () => {
    const chat = "claim-1";
    for (let i = 0; i < 6; i++) ins(chat, { msgId: `${chat}-${i}` });
    const a = messageClaim("sessA", 0, 100);
    const b = messageClaim("sessB", 0, 100);
    ...
    const c = messageClaim("sessC", 0, 100).filter(r => r.chat_id === chat);
  ...
  test("rows below the freshness floor are not claimed", () => {
    ...
    const none = messageClaim("s", Date.now() + 60_000).filter(r => r.id === id);
    expect(none.length).toBe(0);
    // floor in the past → it qualifies
    const got = messageClaim("s", 0).filter(r => r.id === id);
  ...
  test("duplicate idem_key enqueue throws (UNIQUE)", () => {
    outboundCreate({ kind: "reply", idem_key: "uniq-1", chat_id: "x", thread_type: "user", payload: "{}" });
    expect(() => outboundCreate({ kind: "reply", idem_key: "uniq-1", chat_id: "x", thread_type: "user", payload: "{}" })).toThrow();
  });
  ...
  test("watermark mark-processed leaves newer rows unprocessed", () => {
    ...
    const out = outboundCreate({ kind: "reply", idem_key: "wm-out", chat_id: chat, thread_type: "user", watermark_id: id2, payload: "{}" });
    outboundUpdate(out, "sent", { sentIds: [1] }, { chatId: chat, watermarkId: id2 });
    expect(outboundGet(out)?.status).toBe("sent");
    // id1,id2 processed; id3 (> watermark) still unprocessed.
    const unproc = messageList(chat, id3).map(r => r.id);
    expect(unproc).toContain(id3);
    expect(unproc).not.toContain(id1);
    expect(unproc).not.toContain(id2);
  });

  test("messageUpdate persists local_path onto the row", () => {
    ins("lp-1", { msgId: "lp-a" });
    const row = messageGet("lp-a")!;
    messageUpdate(row.id, "/tmp/x.jpg");
    expect(messageGet("lp-a")?.local_path).toBe("/tmp/x.jpg");
  });
});
